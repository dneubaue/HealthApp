...
- (void)viewDidLoad
{
    ...
    // -- NUTRITION VIEW --
    nutritionView = [[UIView alloc] initWithFrame:CGRectMake(0, 64,
...
//                                initWithFrame:CGRectMake(0, nutritionView.frame.size.height-268,
...
//    [scrollView setContentSize:CGSizeMake(nutritionView.frame.size.width*2, 248)];
//    [nutritionView addSubview:scrollView];
    
//    CGRect pageControlView = CGRectMake(0, self.view.frame.size.height-20, self.view.frame.size.width, 20);
...
//    [self.view addSubview:pageControl];
    ...
    // Table
    CGRect nutriTableFrame = CGRectMake(0, nutritionView.frame.size.height-268, nutritionView.frame.size.width, 248);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.tag == DIARY_TAG) {
        ...
        NutritonCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
        if (!cell) {
            cell = [[NutritonCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void)setUpWaterGlasses
{
    int waterGlasses = [[FoodStore sharedStore] getGlassesOfWater];
    for (int i = 0; i < 8; i++) {
        if (i < waterGlasses) {
            UIImageView *glassOfWater = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"waterGlass"]];
            [glassOfWater setFrame:CGRectMake(36 + 32*i, 81, 24, 24)];
            [waterView addSubview:glassOfWater];
        }
        else {
            if (i == 7) {
                newWaterGlass = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"newWater"]];
                [newWaterGlass setFrame:CGRectMake(36 + 32*i, 78, 30, 30)];
                [waterView addSubview:newWaterGlass];
                
                newWaterButton = [[UIButton alloc] initWithFrame:CGRectMake(36 + 32*i, 78, 30, 30)];
                [newWaterButton addTarget:self
                                   action:@selector(addGlassOfWater:)
                         forControlEvents:UIControlEventTouchUpInside];
                [waterView addSubview:newWaterButton];
            }
            else {
                UIImageView *glass = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"glass"]];
                [glass setFrame:CGRectMake(36 + 32*i, 81, 24, 24)];
                [waterView addSubview:glass];
            }
        }
    }
...
- (void)addGlassOfWater:(id)sender
{
    int waterGlasses = [[FoodStore sharedStore] glassesOfWater];
    
    if (waterGlasses == 7) {
        [newWaterGlass removeFromSuperview];
        [newWaterButton removeFromSuperview];
    }
    
    UIImageView *glassOfWater = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"waterGlass"]];
    [glassOfWater setFrame:CGRectMake(36 + 32*waterGlasses, 81, 24, 24)];
    [waterView addSubview:glassOfWater];
    
    [[FoodStore sharedStore] setGlassesOfWater:waterGlasses+1];
    [amountLabel setText:[NSString stringWithFormat:@"%d", waterGlasses+1]];
}
