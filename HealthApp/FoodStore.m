...
- (id)init
{
    ...
    if (self) {
        ...
        if (!groupLevels) {
            groupLevels = [[NSMutableArray alloc]
...
- (void)incrementFoodGroups:(NSArray *)foodGroups
{
    ...
    CGFloat oldNumber = ((NSNumber *)[groupLevels objectAtIndex:6]).floatValue;
    [groupLevels replaceObjectAtIndex:6 withObject:[NSNumber numberWithFloat:additionalServings+oldNumber]];
...
- (void)deleteFoodNumber:(int)foodNumber andMeal:(int)mealNumber
{
    ...
    CGFloat oldNumber = ((NSNumber *)[groupLevels objectAtIndex:6]).floatValue;
    [groupLevels replaceObjectAtIndex:6 withObject:[NSNumber numberWithFloat:oldNumber-removedServings]];
...
{
    return ((NSNumber *)[groupLevels objectAtIndex:6]).floatValue;
}
...
{
    NSNumber *level = [groupLevels objectAtIndex:0];
    CGFloat totalServings = ((NSNumber *)[groupLevels objectAtIndex:6]).floatValue;
...
- (BOOL)deleteFoodFromCatalog:(NSString *)foodName
{
    BOOL shouldDeleteSection = NO;
    
    // First Letter
    const char *name = [[foodName uppercaseString] UTF8String];
    char firstLetter = name[0];
    int indexOfCatalog = firstLetter - 'A';
    
    // Remove from Food Catalog
    NSMutableArray *section = [foodCatalog objectAtIndex:indexOfCatalog];
    int count = (int)section.count;
    for (int i = 0; i < count; i++) {
        Food *f = [section objectAtIndex:i];
        if ([f.name isEqualToString:foodName]) {
            [section removeObjectAtIndex:i];
            if (section.count == 0) {
                shouldDeleteSection = YES;
            }
            break;
        }
    }
